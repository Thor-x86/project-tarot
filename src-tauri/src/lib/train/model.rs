// Came from one of burn's examples:
// https://github.com/tracel-ai/burn/blob/6b4d5faecce39950fb89dcab64102cd8b1df1191/examples/modern-lstm/src/model.rs
...
#[derive(Module, Debug)]
pub(super) struct StackedLstm<B: Backend> {
	pub layers: Vec<LstmCell<B>>,
}

#[derive(Config, Debug)]
pub(super) struct StackedLstmConfig {
	pub input_size: usize,
	pub hidden_size: usize,
...
impl<B: Backend> StackedLstm<B> {
	...
	pub fn forward(
		...
		let mut states = match states {
			...
			Some(states) => states
				.iter()
				.map(|each| LstmState::<B, 2> {
					cell: each.cell.clone(),
					hidden: each.hidden.clone(),
				})
				.collect::<Vec<_>>(),
		};
...
impl<B: Backend> LstmNetwork<B> {
	...
	/// Returns:
	///     Tuple of
	///      1. Output tensor of shape (batch_size, output_size)
	///      2. LSTM State
	pub fn forward(
		&self,
		x: &Tensor<B, 3>,
		...
		// Forward direction
		let (mut output, mut states) = self.stacked_lstm.forward(x, states);

		(output, states) = match &self.reverse_lstm {
			Some(reverse_lstm) => {
				//Process sequence in reverse direction
				let flipped_x = x.clone().flip([1]);
				let (mut reverse_output, states) = reverse_lstm.forward(&flipped_x, None);
